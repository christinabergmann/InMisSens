...
Hedges' g was `toString(n_feat$estimate)` (SE = `toString(n_feat$se`)
...
Number of features is still significant
...
rma_Distractor <- rma.mv(g_calc, g_var_calc, mods = ~as.factor(object_pair), data = db_ET_MP, random = ~ collapse | short_cite)
  
#summary(rma_Distractor)  
 
aov.type <- anova(rma_Distractor)

sum_eff <- round(coef(summary(rma_TypeFeatures_Lang))[2,], 2)
...
Hedges' g was `toString(sum_eff$estimate)` (SE = `toString(sum_eff$se`)
...
min_fn <- min(mf$mean_age_1)
max_fn <- max(mf$mean_age_1)
...
rma_Distractor <- rma.mv(g_calc, g_var_calc, mods = ~as.factor(object_pair), data = dat_age, random = ~ collapse | short_cite)
  
#summary(rma_Distractor)  

aov.type <- anova(rma_Distractor)

sum_eff <- round(coef(summary(rma_Distractor))[2,], 2)
...
### Plot Distractor Familiarity by age (subset to same age range)

```{r PlotDistFamEffect_age}
...
p <- ggplot(dat_age, aes(mean_age_1/30.44, g_calc, color = condition_label)) + 
...
p
  ...
  jpeg(filename = "figures/Distractor_fam_AgeSubset.jpg", 
       width = 500, height = 300, units = "px")
...
Hedges' g was `toString(type_feat$estimate)` (SE = `toString(type_feat$se`)
