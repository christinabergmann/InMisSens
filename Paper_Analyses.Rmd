...
[Christina: There are some () missing in the following paragraphs, I am a bit tired now, can you check?]
...
During data entry, we noted variation in how the different studies were analyzed, with the time window, offset from naming, and depending variable differing substantially in our sample. For the time window used for statistical analysis we find xxx

The lack of a field standard can have dire consequences, as our analyses show. Depending on which analysis time window (see figure xx) or dependent variable (see figure xx) we focus on, we find support for any of the three possible trajectories of mispronunciation sensitivity. On one hand, this limits the conclusions we can draw. On the other hand, this finding emphasizes that current practices of free, seemingly ad hoc choices regarding data analyses are not sustainable if the field wants to move towards quantitative evidence for theories of language development. 


We take this opportunity to suggest two remedies: Preregistration can serve as proof of a priori decisions regarding data analysis. The peer reviewed form of Registered Reports has already been adopted by a large number of developmental journals, and general journals that publish developmental works, showing the field's increasing acceptance of such practices. Open data can allow others to re-analyze existing datasets to both examine the impact of analysis decisions and cumulatively analyze different datasets in the same way. 


[Christina: Are we submitting to a place with registered reports? We should!]

  + Post-naming time window size analyzed and dependent variable impact misp sensitivity measured
  + Offset time does not impact measurement of misp sensitivity 
...
First aggregation of studies

Advice for experiment design and design to be standardized, ideally preregistered. 

Open science with anonymized data and more complex models might be the future, but without us noting the issues in the field, this cannot happen, so let's call for change now. 
