...
#[CHRISTINA: I prefer exact p-values with more granularity]
...
# [CHRISTINA This should not be necessary]
#cur_dir <- getwd()
...
#[KATIE] Can you also rename the factors here (like t_one)?
#[CHRISTINA] Mpt sire what you man, like in the table? You rename things later on, so why not. 
```
...
We calculated Hedges' *g* using the raw means and standard deviations reported in the paper (*n* = `r with(dat, n_distinct(short_cite[es_method == "group_means_one" | es_method == "group_means_two"]))`) or using reported t-values (*n* = `r with(dat, n_distinct(short_cite[es_method == "t_one" | es_method == "t_two"]))`). Raw means and standard deviations were extracted from figures for `r with(dat, n_distinct(short_cite[ x_from_graph == "yes"]))` papers. In a within-participation design, when two means are compared (i.e. looking during pre- and post-naming) it is necessary to obtain correlations between the two measurements at the participant level to calculate effect sizes and effect size variance based on t-values. One paper reported this correlation [CHRISTINA: Can we cite this??]; we were able to compute correlations using means, standard deviations, and t-values [CHRISTINA: can we add N?] (following Csibra, et al. 2016, Appendix B; see also Rabagliati, Ferguson, & Lew-Williams, 2018). Correlations were imputed for the remaining papers (see Black & Bergmann, 2017, for the same procedure). We could compute a total of `r sum(dat$is_mp == 0)` effect sizes for correct pronunciations and `r sum(dat$is_mp == 1)` for mispronunciations.
...
[CHRISTINA: CAn you add the stats for the non-sig results?]
...
We then examined the interaction between age and mispronunciation sensitivity (correct vs. mispronounced words) in our whole dataset. The moderator test was significant (QM(`r r2(rma_MPeffect_age$m)`) = `r r2(rma_MPeffect_age$QM)`, *p* `r psig(rma_MPeffect_age$QMp)`). This result is in line with the general observation that as infants mature they become better at language processing. The interaction between age and mispronunciation sensitivity, however, was not significant $\beta$ = `r r2(sum_eff$estimate)`, (SE = `r r2(sum_eff$se)`, 95% CI [`r r2(sum_eff$ci.lb)`, `r r2(sum_eff$ci.ub)`], *p* `r psig(sum_eff$pval)`), suggesting that as infants age, their mispronunciation sensitivity remains the same.
...
(BUT I WONDER WHETHER WE COULD ENCODE THE REPORTED INTERACTION TERMS AND THE CORRELATION AND THEN DO SOMETHING WITH THAT?)
...
[CHRISTINA] I PREFER NUMBER OF FEATURES TO SIZE SOMEHOW 
...
p
  ...
  #[CHRISTINA] AGAIN, WHAT DO YOU THINK OF VIOLINS?
...
[CHRISTINA] I WOULD NOT OVERINTERPRET IT, THE ESTIMATE DID NOT DRAMATICALLY CHANGE, AND IT IS BARELY SIGNIFICANT. DOES THAT HELP?
