...
We identified two sets of variables which varied across papers to assess the influence of data analysis choices on resulting effect size: the time course of a trial and time window analyzed and which dependent variable(s) were reported. In the following, we discuss the possible theoretical motivation for these data analysis choices, the variation present in the current meta-analysis dataset, and the influence these analysis choices have on mispronunciation sensitivity development.
...
[Christina] Ideally load all libraries in the beginning of the doc. (reshape)
...
We first assessed whether size of post-naming phase analyzed had an impact on the size of the reported mispronunciation sensitivity. First, we calculated the meta-analytic effect for object identification in response to mispronounced target words/images, including post-naming window size as a moderator.

[Christina] the next bit is weird, in the pdf it's significant, but you write it's not. Also, I think the beta is quite large anbd the CI reasonably distinct from 0. 


The moderator test was not significant `r mod_test(rma_post_dur)` and the estimate for post-naming phase size was relatively small, $\beta$`r full_estimate(rma_post_dur_eff)`. 
...
This suggests that upon hearing a mispronunciation, infants' looks to the target image were similar regardless of the size of the post-naming phase analyzed. We next assessed whether post-naming phase size was related to mispronunciation sensitivity. 
...
We next assessed whether the time between the target was named and the start of the analysis, namely offset time, had an impact on the size of the reported mispronunciation sensitivity. First, we calculated the meta-analytic effect size for object identification in response to mispronounced target words/images, including offset time as a moderator. 

[Christina] again, the pdf tells me sth different

The moderator test was not significant `r mod_test(rma_offset)` and the estimate for offset time was very small, $\beta$`r full_estimate(rma_offset_eff)`. 

[Christina] Again, we didn't test, so say sth like "did not differ significantly." 

This suggests that upon hearing a mispronunciation, infants' looks to the target image were similar regardless of the offset time used to analyze the data. We next assessed whether offset time was related to mispronunciation sensitivity. 

[Christina] Again, rewording suggestion: "When investigating mispronunciation sensitivity by including condition, the moderator test with was ... 

We merged the two datasets and included condition (correct pronunciation, mispronunciation) as an additional moderator. The moderator test was significant, `r mod_test(rma_post_dur_MS)`, but the estimate for the interaction between offset time and condition was almost zero $\beta$`r full_estimate(rma_offset_MS_eff)`. 
...
The Difference Score and Pre vs. Post can be considered similar to one another, in that they are calculated on the same type of data and consider pre-naming preferences. The Post dependent variable, in contrast, does not consider pre-naming preferences. 

[Christina] Actually, in other domains (eeg, longitudinal studies, RCTs) people tlak a lot about whether or not to consider a baseline, so this is not strictly true. 
