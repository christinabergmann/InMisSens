...
# This script creates boxplots for the original data from Swingley & Aslin, 2000
...
misp <- rnorm(100, mean=0.612614286, sd=0.093135265)

cm <- data.frame(cbind(correct, misp))
...
box1 <- ggplot(data=dat.m2, aes(x=condition, y=PTL, fill=condition)) +
  geom_boxplot()+
  scale_fill_manual(values=cbPalette)+
  theme_bw()+
  annotate("segment", x = .67, xend = .67, y = .7, yend = .5, colour="black", size=1, arrow=arrow())+
  annotate("segment", x = 1, xend = 2, y = .96, yend = .96, colour="black", size=1)+
  annotate("text", x = .5, y = .63, parse = TRUE, label = "recognition",angle = 90, size = 10)+
  annotate("text", x = 1.5, y = .99, parse = TRUE, label = "misp_sensitivity", size = 10)+
...
#ggsave("Example_boxplot.jpg", box1)

dat.m2$condition <- ifelse(dat.m2$condition == "correct", "Correct", "Misp")
