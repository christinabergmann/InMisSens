---
title: "MP Publication Bias"
author: "originally Christina Bergmann, modified by Katie Von Holzen"
output: 
  pdf_document:
    toc: true
---
...
Use ranktest() to check for funnel plot asymmetry. All three tests show evidence of significant asymmetry.

```{r FunnelPlotAsymmetry, echo = FALSE}
...
These files can be found in "p_curve_app/"

for correct: p_curve_co.txt
for mispronounced: p_curve_mp.txt

Just copy and paste the .txt onto the website and voila!

NOTE: This will always append lines to the existing txt file, check for double writing!
...
# What is the power of the studies in the dataset?


```{r Power_exist, echo = FALSE}
d = rma.mv(g_calc, g_var_calc, data = db_mpe, random = ~ 1 | short_cite)$b[,1]

# estimate from mispronunciation sensitivity
mp_est <- coef(summary(rma_MPeffect))[2,]$estimate

pwr.t.test(n = median(db_mpe$n_1, na.rm=TRUE), d = mp_est, sig.level = .05, type = "paired", alternative = "two.sided")

```

# What is the average power to detect a mispronunciation sensitivity effect (to be compared with p-curve estimate)
